...
public class TransactionController {
    ...
    @GetMapping
    public Page<TransactionResponse> all(
            @ModelAttribute TransactionFilterRequest f, Pageable p, @RequestAttribute(ACTOR_ATTRIBUTE) ExpenseActor a) {
        return service.findAll(f, p, a);
    }
    @GetMapping("/{id}")
    public TransactionResponse one(@PathVariable Long id, @RequestAttribute(ACTOR_ATTRIBUTE) ExpenseActor a) {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> delete(@PathVariable Long id, @RequestAttribute(ACTOR_ATTRIBUTE) ExpenseActor a) {
        service.delete(id, a);
