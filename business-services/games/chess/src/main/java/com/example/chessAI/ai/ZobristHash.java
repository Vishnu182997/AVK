...
public class ZobristHash {
  ...
  private static final long[] EN_PASSANT_KEYS = new long[64];
    ...
    for (int color = 0; color < 2; color++) {
      for (int piece = 0; piece < 6; piece++) {
        for (int square = 0; square < 64; square++) {
          PIECE_KEYS[color][piece][square] = random.nextLong();
        }
      }
    }
  ...
  public static long generateHash(ChessBoard board) {
    ...
    for (int row = 0; row < ChessBoard.SIZE; row++) {
      for (int col = 0; col < ChessBoard.SIZE; col++) {
        ...
        int pieceIndex = getPieceIndex(piece.getType());

        int square = row * 8 + col;

        hash ^= PIECE_KEYS[colorIndex][pieceIndex][square];
      }
    }
   ...
   */
  private static int getPieceIndex(PieceType type) {
    switch (type) {
      case PAWN:
        return 0;
