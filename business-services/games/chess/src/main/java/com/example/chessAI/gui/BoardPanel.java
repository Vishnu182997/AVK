...
public class BoardPanel extends JPanel {
  ...
  public String getLastMoveAnnouncement() {
    return lastMoveAnnouncement;
  }
  ...
  public void setGameMode(GameMode gameMode) {
    if (gameMode == null || this.gameMode == gameMode) {
      return;
    }
  ...
  public void setHumanColor(Color humanColor) {
    if (humanColor == null || this.humanColor == humanColor) {
      return;
    }
  ...
  public void undoMove() {
    ...
    for (int i = 0; i < movesToUndo; i++) {
      ...
      if (!appliedMoveAnnouncements.isEmpty()) {
        appliedMoveAnnouncements.remove(appliedMoveAnnouncements.size() - 1);
      }
    }
    gameOver = false;
    engineError = null;
  ...
  public boolean attemptMove(Move move) {
    if (!canHumanInteract() || move == null || !moveValidator.isValidMove(board, move)) {
      return false;
    }
  ...
  private void updateLastMoveSquares() {
    for (int row = 0; row < BOARD_SIZE; row++) {
      for (int col = 0; col < BOARD_SIZE; col++) {
        squares[row][col].setLastMoveFrom(
            lastMove != null && lastMove.getFromRow() == row && lastMove.getFromCol() == col);
        squares[row][col].setLastMoveTo(
            lastMove != null && lastMove.getToRow() == row && lastMove.getToCol() == col);
      }
    }
  }
  ...
  private String completedMoveAnnouncement(Color mover, Move move, boolean computerMove) {
    ...
    return actor + " moved from " + squareName(move.getFromRow(), move.getFromCol()) + " to "
        + squareName(move.getToRow(), move.getToCol()) + ".";
  }
  ...
  private boolean canHumanInteract() {
    return !gameOver && !aiThinking && engineError == null
        && isHumanControlled(board.getSideToMove());
  }
  ...
  private Move findMatchingLegalMove(Move requestedMove) {
    for (Move legalMove : moveGenerator.generateLegalMoves(board, board.getSideToMove())) {
      if (sameCoordinates(legalMove, requestedMove)) {
        return legalMove;
      }
    }
    return null;
  ...
  private boolean sameCoordinates(Move left, Move right) {
    return left.getFromRow() == right.getFromRow() && left.getFromCol() == right.getFromCol()
        && left.getToRow() == right.getToRow() && left.getToCol() == right.getToCol();
  }

  private PieceType resolvePromotionPiece(PieceType requested) {
    if (requested == PieceType.QUEEN || requested == PieceType.ROOK || requested == PieceType.BISHOP
        || requested == PieceType.KNIGHT) {
      return requested;
    }
  ...
  private PieceType showPromotionDialog() {
    ...
    PieceType selected =
        (PieceType) JOptionPane.showInputDialog(this, "Choose promotion piece:", "Pawn Promotion",
            JOptionPane.QUESTION_MESSAGE, null, choices, PieceType.QUEEN);
  ...
  private void scheduleAiTurnIfNeeded() {
    if (gameMode != GameMode.HUMAN_VS_AI || gameOver || aiThinking || engineError != null) {
      return;
    }
    if (board.getSideToMove() == humanColor) {
      return;
    }
    if (moveGenerator.generateLegalMoves(board, board.getSideToMove()).isEmpty()) {
      return;
    }
      ...
      protected void done() {
        ...
        try {
          ...
          aiDelayTimer = new Timer(
              AI_VISUAL_DELAY_MILLIS, e -> applyAiMove(bestMove, requestGameId, requestId));
  ...
  private void applyAiMove(Move bestMove, String requestGameId, String requestId) {
    ...
    if (legalMove.isPromotion()) {
      legalMove.setPromotionPiece(resolvePromotionPiece(bestMove.getPromotionPiece()));
    }
    applyLegalMove(legalMove, true);
  }

  private boolean isCurrentAiRequest(String requestGameId, String requestId) {
    return requestGameId.equals(gameId) && requestId.equals(activeAiRequestId)
  ...
  private void updateStatus() {
    ...
    if (gameOver && inCheck && noMoves) {
      statusLabel.setText(withLastMoveAnnouncement(side + " is checkmated. "
          + (turn.opposite() == Color.WHITE ? "White" : "Black") + " wins."));
    ...
    } else if (gameMode == GameMode.HUMAN_VS_AI && !engineReady
        && board.getSideToMove() != humanColor) {
