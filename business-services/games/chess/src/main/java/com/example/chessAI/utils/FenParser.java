...
public class FenParser {
   ...
   */
  private static void loadPieces(ChessBoard board, String position) {
    ...
    for (int row = 0; row < 8; row++) {
      ...
      for (int i = 0; i < rows[row].length(); i++) {
        ...
        if (Character.isDigit(symbol)) {
          int empty = Character.getNumericValue(symbol);

          col += empty;

        } else {
          squares[row][col] = createPiece(symbol);
   ...
   */
  private static Piece createPiece(char symbol) {
  ...
  public static String toFEN(ChessBoard board) {
    ...
    fen.append(board.getSideToMove() == Color.WHITE ? "w" : "b");
