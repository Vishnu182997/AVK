...
public class Minimax {
   ...
   */
  public int search(ChessBoard board, int depth, boolean maximizingPlayer) {
    /*
     * Stop condition
     */
    if (depth == 0) {
    ...
    Color side = maximizingPlayer ? Color.WHITE : Color.BLACK;
    ...
    if (maximizingPlayer) {
      ...
      for (Move move : moves) {
        ...
        int score = search(board, depth - 1, false);
      ...
      for (Move move : moves) {
        ...
        int score = search(board, depth - 1, true);
   ...
   * Finds the best move.
   */
  public Move findBestMove(ChessBoard board, Color color, int depth) {
    List<Move> moves = moveGenerator.generateLegalMoves(board, color);

    Move bestMove = null;

    int bestScore = color == Color.WHITE ? Integer.MIN_VALUE : Integer.MAX_VALUE;

    for (Move move : moves) {
      board.makeMove(move);

      int score = search(board, depth - 1, color == Color.BLACK);
