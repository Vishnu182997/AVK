...
public class Pawn extends Piece {
  ...
  public List<Move> getLegalMoves(ChessBoard board, int row, int col) {
    ...
    if (board.isValidSquare(next, col) && board.getPiece(next, col) == null) {
      addPawnMove(row, col, next, col, moves);

      int startRow = color == Color.WHITE ? 6 : 1;
      int doubleRow = row + (2 * direction);
      if (row == startRow && board.isValidSquare(doubleRow, col)
  ...
  private void addCapture(
      ChessBoard board, int row, int col, int targetRow, int targetCol, List<Move> moves) {
    ...
    if (enPassantSquare != null && enPassantSquare.getRow() == targetRow
