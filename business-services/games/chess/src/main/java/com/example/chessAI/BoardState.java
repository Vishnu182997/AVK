...
public class BoardState {
  ...
  public BoardState(ChessBoard chessBoard) {
    ...
    whiteKingSideCastle = chessBoard.canWhiteCastleKingSide();

    whiteQueenSideCastle = chessBoard.canWhiteCastleQueenSide();

    blackKingSideCastle = chessBoard.canBlackCastleKingSide();

    blackQueenSideCastle = chessBoard.canBlackCastleQueenSide();

    if (chessBoard.getEnPassantSquare() != null) {
      enPassantSquare = new Position(chessBoard.getEnPassantSquare());
    }

    halfMoveClock = chessBoard.getHalfMoveClock();

    fullMoveNumber = chessBoard.getFullMoveNumber();
  }
  ...
  public void restore(ChessBoard chessBoard) {
    ...
    for (int row = 0; row < ChessBoard.SIZE; row++) {
      for (int col = 0; col < ChessBoard.SIZE; col++) {
        ...
        if (board[row][col] != null) {
          target[row][col] = copyPiece(board[row][col]);
        }
      }
    }
    ...
    chessBoard.setEnPassantSquare(enPassantSquare == null ? null : new Position(enPassantSquare));
