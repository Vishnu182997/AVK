...
public class PGNParser {
   ...
   */
  public static PGNGame load(String fileName) throws IOException {
    ...
    game.setMoves(cleanMoves(moves.toString()));
   ...
   * [White "Kasparov"]
   */
  private static void parseHeader(PGNGame game, String line) {
    ...
    String key = line.substring(1, space);

    String value = line.substring(space + 1).replace("\"", "").replace("]", "");
   ...
   * Saves PGN file.
   */
  public static void save(PGNGame game, String fileName) throws IOException {
    BufferedWriter writer = new BufferedWriter(new FileWriter(fileName));

    for (String key : game.getHeaders().keySet()) {
      writer.write("[" + key + " \"" + game.getHeaders().get(key) + "\"]");
   ...
   */
  public static String moveToPGN(Move move, ChessBoard board) {
    if (move == null) {
      return "";
    }
    ...
    if (piece != null) {
      ...
      if (type != PieceType.PAWN) {
        notation.append(pieceLetter(type));
      }
    }

    if (move.isCapture()) {
      if (piece != null && piece.getType() == PieceType.PAWN) {
        notation.append((char) ('a' + move.getFromCol()));
      }

      notation.append("x");
    }

    notation.append(squareName(move.getToRow(), move.getToCol()));
