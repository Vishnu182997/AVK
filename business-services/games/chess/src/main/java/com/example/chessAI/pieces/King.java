...
public class King extends Piece {
  ...
  private void addCastlingMoves(ChessBoard board, int row, int col, List<Move> moves) {
    if (hasMoved || col != 4 || board.isKingInCheck(color)) {
      return;
    }

    boolean kingSide =
        color == Color.WHITE ? board.canWhiteCastleKingSide() : board.canBlackCastleKingSide();
    boolean queenSide =
        color == Color.WHITE ? board.canWhiteCastleQueenSide() : board.canBlackCastleQueenSide();
    ...
    if (queenSide && canCastle(board, row, 3, 2, 0) && board.getPiece(row, 1) == null) {
  ...
  private boolean canCastle(
      ChessBoard board, int row, int throughCol, int destinationCol, int rookCol) {
    Piece rook = board.getPiece(row, rookCol);
    if (rook == null || rook.getColor() != color || rook.getType() != PieceType.ROOK
        || rook.hasMoved()) {
      return false;
    }

    if (board.getPiece(row, throughCol) != null || board.getPiece(row, destinationCol) != null) {
      return false;
    }
