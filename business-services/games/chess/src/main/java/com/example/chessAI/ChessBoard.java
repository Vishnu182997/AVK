...
public class ChessBoard {
  ...
  public void makeMove(Move move) {
    ...
    if (move.isPromotion() && piece != null) {
      PieceType promotionType =
          move.getPromotionPiece() == null ? PieceType.QUEEN : move.getPromotionPiece();
    ...
    halfMoveClock = pawnMove || capturedPiece != null ? 0 : halfMoveClock + 1;
  ...
  private void updateEnPassantSquare(Move move, Piece piece) {
    ...
    if (piece != null && piece.getType() == PieceType.PAWN
      ...
      enPassantSquare = new Position((move.getFromRow() + move.getToRow()) / 2, move.getFromCol());
    }
  }
  ...
  public void setCastlingRights(boolean whiteKingSideCastle, boolean whiteQueenSideCastle,
  ...
  public boolean isKingInCheck(Color color) {
    ...
    return isSquareAttacked(kingPosition.getRow(), kingPosition.getCol(), color.opposite());
  }
  ...
  public boolean isSquareAttacked(int row, int col, Color attackingColor) {
    ...
    for (int[] offset : knightOffsets) {
      if (isAttackingPiece(row + offset[0], col + offset[1], attackingColor, PieceType.KNIGHT)) {
        return true;
      }
    }

    int[][] diagonalDirections = {{1, 1}, {1, -1}, {-1, 1}, {-1, -1}};
    if (isAttackedAlongDirections(
            row, col, attackingColor, diagonalDirections, PieceType.BISHOP, PieceType.QUEEN)) {
      return true;
    }

    int[][] straightDirections = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};
    if (isAttackedAlongDirections(
            row, col, attackingColor, straightDirections, PieceType.ROOK, PieceType.QUEEN)) {
      return true;
    }
    ...
    for (int rowOffset = -1; rowOffset <= 1; rowOffset++) {
      for (int colOffset = -1; colOffset <= 1; colOffset++) {
        ...
        if (isAttackingPiece(row + rowOffset, col + colOffset, attackingColor, PieceType.KING)) {
          return true;
        }
      }
    }
  ...
  private boolean isAttackingPiece(int row, int col, Color attackingColor, PieceType type) {
    Piece piece = getPiece(row, col);
    return piece != null && piece.getColor() == attackingColor && piece.getType() == type;
  }

  private boolean isAttackedAlongDirections(int row, int col, Color attackingColor,
      int[][] directions, PieceType firstType, PieceType secondType) {
