...
public class EndgameTablebase {
   ...
   */
  public boolean supports(ChessBoard board) {
   ...
   */
  public Move getBestMove(ChessBoard board, Color side) {
    if (!supports(board)) {
      return null;
    }
     ...
     * King + Queen vs King
     */
    Move move = findQueenMateMove(board, side);
   ...
   */
  private Move findQueenMateMove(ChessBoard board, Color color) {
    Piece queen = findPiece(board, color, PieceType.QUEEN);
    ...
    for (Move move : moves) {
      if (move.getMovedPiece() != null && move.getMovedPiece().getType() == PieceType.QUEEN) {
        return move;
      }
    }
   ...
   */
  private Move findRookMove(ChessBoard board, Color color) {
    Piece rook = findPiece(board, color, PieceType.ROOK);
    ...
    for (Move move : moves) {
      if (move.getMovedPiece() != null && move.getMovedPiece().getType() == PieceType.ROOK) {
        return move;
      }
    }
   ...
   */
  private int countPieces(ChessBoard board) {
    int count = 0;
   ...
   */
  private Piece findPiece(ChessBoard board, Color color, PieceType type) {
    Piece[][] pieces = board.getBoard();
