...
class TicketControllerTest {
  ...
  void exceptionHandlerPreservesNotFoundAndForbiddenStatus() {
    ...
    assertEquals(404,
        handler.handleTicketException(new TicketException("missing", 404)).getStatusCodeValue());
    assertEquals(403,
        handler.handleTicketException(new TicketException("forbidden", 403)).getStatusCodeValue());
  }
  ...
  void exceptionHandlerPreservesInvalidStatusResponse() {
    TicketExceptionHandler handler = new TicketExceptionHandler();
    assertEquals(400,
        handler.handleTicketException(new TicketException("invalid transition", 400))
            .getStatusCodeValue());
  }
}
