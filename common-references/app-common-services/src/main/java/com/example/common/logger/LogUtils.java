...
public class LogUtils {
    ...
    private static final ThreadLocal<String> currentLambdaName = new ThreadLocal<>();
    private static final ThreadLocal<Map<String, Object>> currentRequestData = new ThreadLocal<>();
...
//			((Exception)throwable).printStackTrace();
	...
	public static void infoLogger(LambdaLogger lambdaLogger, LoggerMessageDTO loggerMessage) {
		if ("true".equalsIgnoreCase(System.getenv("PRINT_LOG"))) {
			printDebuggerLogs(lambdaLogger, FastCollabCommonUtil.getResponseBody(loggerMessage), INFO_LOG, "API_LOGS");
//		if(lambdaLogger!=null)
...
//			System.out.println("Debug Logger: "+FastCollabCommonUtil.getResponseBody(loggerMessage));
	...
	public static void customRequestLogger(LambdaLogger lambdaLogger, String inputResource, LoggerMessageDTO loggerMessage) {
		String inputRequestTitle = FastCollabCommonUtil.hasData(inputResource) ? "Input Request for " + inputResource +": " : "Input Request: ";		
		printDebuggerLogs(lambdaLogger, (inputRequestTitle + FastCollabCommonUtil.getResponseBody(loggerMessage)), INFO_LOG, API_LOGS);
...
//			System.out.println(inputRequestTitle + FastCollabCommonUtil.getResponseBody(loggerMessage));
	...
	public static void customResponseLogger(LambdaLogger lambdaLogger, LoggerMessageDTO loggerMessage) {
		if ("true".equalsIgnoreCase(System.getenv("PRINT_LOG"))) {
			printDebuggerLogs(lambdaLogger, (FastCollabCommonUtil.getResponseBody(loggerMessage)), INFO_LOG, "OUTPUT RESPONSE");
...
//			System.out.println("OutPut Response: "+FastCollabCommonUtil.getResponseBody(loggerMessage));
	...
	public static void customSQLQueryLogger(LambdaLogger lambdaLogger, LoggerMessageDTO loggerMessage) {
		if ("true".equalsIgnoreCase(System.getenv("PRINT_LOG"))) {
			printSqlLogs(lambdaLogger, (FastCollabCommonUtil.getResponseBody(loggerMessage)), INFO_LOG, SQL_QUERY);
//		if(lambdaLogger!=null)
...
//			System.out.println("SQL Query Logger: "+FastCollabCommonUtil.getResponseBody(loggerMessage));
	...
	public static void thirdPartyApiLogger(LambdaLogger lambdaLogger, LoggerMessageDTO loggerMessage) {
		if ("true".equalsIgnoreCase(System.getenv("PRINT_LOG"))) {
			printDebuggerLogs(lambdaLogger, (FastCollabCommonUtil.getResponseBody(loggerMessage)), INFO_LOG, THIRD_PARTY_API);
//		if(lambdaLogger!=null)
...
//			System.out.println("Custom Logger: "+FastCollabCommonUtil.getResponseBody(loggerMessage));
    ...
    /**
     * Set current lambda name for the request
     */
    public static void setCurrentLambdaName(String lambdaName) {
        currentLambdaName.set(lambdaName);
    }

    /**
     * Get current lambda name
     */
    public static String getCurrentLambdaName() {
        return currentLambdaName.get();
    }

    /**
     * Clear lambda name after request completion
     */
    public static void clearLambdaName() {
        currentLambdaName.remove();
    }

    /**
     * Store request data for later retrieval
     */
    public static void setCurrentRequestData(Map<String, Object> requestData) {
        currentRequestData.set(requestData);
    }

    /**
     * Get request data of current request
     */
    public static Map<String, Object> getCurrentRequestData() {
        return currentRequestData.get();
    }

    /**
     * Clear request data after request completion
     */
    ...
    /**
     * Set current user id for the request
     */
    ...
    /**
     * Clear user id after request completion
     */
    ...
    private static Object summarizeObject1(Object obj) {
     ...
     */
    private static Map<String, Object> summarizeMap1(Map<?, ?> map) {
    ...
    private static Map<String, Object> summarizeMap(Map<?, ?> input) {
        ...
        for (Map.Entry<?, ?> entry : input.entrySet()) {
            ...
            if ("body".equalsIgnoreCase(key)
                    || "requestBody".equalsIgnoreCase(key)
                    || "responseBody".equalsIgnoreCase(key)
                    || "payload".equalsIgnoreCase(key)) {
                result.put(key + "Size", value == null ? 0 : String.valueOf(value).length());
                result.put(key, "REMOVED");
                continue;
            }

            if (value instanceof Map) {
                result.put(key, summarizeMap((Map<?, ?>) value));
            } else {
                result.put(key, summarizeObject(value));
            }
        }
...
//            return truncateMessage(logEntry, MAX_LOG_SIZE);
    ...
    private static String createStructuredLog1(String level, String action, Object message) {
        try {
            Map<String, Object> logEntry = new HashMap<>();
            String correlationId = getCurrentCorrelationId();
            ...
            if (message instanceof Map) {
                safeMessage = summarizeMap((Map<?, ?>) message);
            } else if (message instanceof LoggerMessageDTO) {
                Map<String, Object> dtoMap = objectMapper.convertValue(message, Map.class);
                safeMessage = summarizeMap(dtoMap);
            } else {
                safeMessage = summarizeObject(message);
            }

            logEntry.put("data", safeMessage);
    ...
    public static void startRequest(LambdaLogger lambdaLogger, String resource, Map<String, Object> requestData) {
        ...
        // store request data for later use (e.g., exception logging)
        setCurrentRequestData(requestData);
        
        String logEntry = createStructuredLog(INFO_LOG, "REQUEST_START", startData);
        
        if (FastCollabCommonUtil.hasData(resource)) {
        	logEntry = truncateRequestData(logEntry);
        }
    ...
    @SuppressWarnings("unchecked")
	private static String truncateRequestData(String logEntry) {
		try {
			Map<String, Object> logMap = objectMapper.readValue(logEntry, Map.class);

			Object dataObj = logMap.get("data");
			...
			Object requestDataObj = data.get("requestData");
			if (!(requestDataObj instanceof Map)) {
				return logEntry;
			}

			Map<String, Object> requestData = (Map<String, Object>) requestDataObj;

			String path = String.valueOf(requestData.get("path"));

			if (!Arrays.asList("/tripbox/v1/list", "/tripbox/v1/stats").contains(path)) {
				return logEntry;
			}
			...
			requestData.put("bodySize", body.length());

			byte[] bytes = body.getBytes(StandardCharsets.UTF_8);

			if (bytes.length > MAX_LOG_SIZE) {
				requestData.put("body", new String(bytes, 0, MAX_LOG_SIZE, StandardCharsets.UTF_8) + "...TRUNCATED");
			}
    ...
    public static void endRequest(LambdaLogger lambdaLogger, String resource, Object response, long executionTime) {
        ...
        // clear thread-local request data at the end of the request
        clearRequestData();
	...
	public static void exceptionLogger(LambdaLogger lambdaLogger, Throwable throwable, String context) {
		...
		try {
			...
			exceptionLog.put("correlationId", getCurrentCorrelationId());
			exceptionLog.put("lambdaName", getCurrentLambdaName());
			exceptionLog.put("requestData", getCurrentRequestData());
			exceptionLog.put("userId", getCurrentUserId());
			exceptionLog.putAll(errorData);
			String errorMessage = throwable.getMessage();
			if (!"Flights are still loading please wait for some time".equalsIgnoreCase(errorMessage)
					|| !"Hotels are still loading please wait for some time".equalsIgnoreCase(errorMessage)
					|| !"Not Called".equalsIgnoreCase(errorMessage)
					|| !errorMessage.contains("Unrecognized token 'WARMUP_MESSAGE'")) {
				// Send the captured exception details to an external Python Lambda
				PythonExceptionLogger.publish(exceptionLog, lambdaLogger);
			}
		} catch (Exception e) {
			System.out.println("issue capturing complete logs with python");
		}
	}
