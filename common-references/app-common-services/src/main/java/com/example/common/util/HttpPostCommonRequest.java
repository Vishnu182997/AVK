...
public class HttpPostCommonRequest implements Serializable {
		...
		try {
			...
			if(responseByte == null) {
				...
				if(responseByte != null && handleErrorResponse) {
					...
					LogUtils.printlogger(lambdaLogger, "error code:" + conn.getResponseCode() + " error response: " +  responseByte != null ? new String(responseByte) : null);
					throw new CustomException("Failed : HTTP error code : " + conn.getResponseCode() + " Error Response: " + responseByte, FCHttpStatus.BAD_REQUEST, CommonStatusCode.FAILED);
				}
			}
			...
			conn.disconnect();
			LogUtils.printlogger(lambdaLogger, "Response Code: " + conn.getResponseCode() + " Request Url: " + endPointUrl + " Response: " + response);
		} catch (Exception e) {
    ...
    public static String httpRequest(String endPointUrl, Map<String, String> headers, String input, String requestMethodType, LambdaLogger lambdaLogger, boolean handleErrorResponse, Environment environment) throws IOException {
		int timeOut ;
		if(AvkCommonUtil.hasData(environment.getProperty("DOTREZ_TIME_OUT"))){
    ...
    public static String httpRequestBelairProxy(String endPointUrl, Map<String, String> headers, String requestBody, String requestMethodType, LambdaLogger lambdaLogger, boolean handleErrorResponse, Environment environment) throws IOException {
		...
		try {
            ...
            byte[] responseByte = readByte(connection.getInputStream());
            if(responseByte == null) {
            	responseByte = readByte(connection.getErrorStream());
				if(responseByte != null && handleErrorResponse) {
					response = new String(responseByte);
				} else {
					LogUtils.printlogger(lambdaLogger, "error code:" + connection.getResponseCode() + " error response: " +  responseByte != null ? new String(responseByte) : null);
					throw new CustomException("Failed : HTTP error code : " + connection.getResponseCode() + " Error Response: " + responseByte, FCHttpStatus.BAD_REQUEST, CommonStatusCode.FAILED);
				}
			}
