 ...
 * 
 * This is a Frontend Handler for invoking the AWS Lambda.
 *
 */
public class RequestDispatcherHandler implements RequestHandler<AWSProxyRequest, AWSProxyResponse>{
	...
	public AWSProxyResponse handleRequest(AWSProxyRequest input, Context context) {
		try {
			if(ctx==null) {
				ctx = new AnnotationConfigApplicationContext(JPAConfiguration.class);
				LogUtils.printlogger(context.getLogger(), "Context Initiazing............");
			}
			RequestHandlerService<AWSProxyResponse> service = ctx.getBean(RequestHandlerService.class);
			
			LogUtils.printlogger(context.getLogger(), "REQUEST SERVED");
			
			if(AvkConstants.WARMUP_MESSAGE.equalsIgnoreCase(input.getBody())) {
			...
			return new AWSProxyResponse(ex.getPayLoad(), ex.getMessage(), ex.getStatusCode(), ex.getErrorCode(), ex.isIsError());
		} catch (Exception ex) {
			LogUtils.exceptionLogger(context.getLogger(), ex);
